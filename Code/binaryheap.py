...
class BinaryMinHeap(object):
    ...
    def replace_min(self, item):
        ...
        if self.size() > 1:
            self._bubble_down(0)
        return min_item
    # in bubble up you are rearranging from leaf(down) to up
    ...
    # in bubble down you are rearranging from index to leaf(down)
    def _bubble_down(self, index):
        ...
        out of order. Maximum path length in complete binary tree is log n."""
        # Process:
        #   * In case it is outside the scope of the list, return invalid index.
        #   * 
        #   *
        #   * 
        if not (0 <= index <= self._last_index()):
        ...
        # if self.items[child_index] > self.items[]
